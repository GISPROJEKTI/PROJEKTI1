...
=======
			...
			if (this.activeQuestion.type == 5){
					answerSelector = "input:text";
					answerSelector2 = "input:checked";
			}
            // Make sure user has answered something
            var answerVal = this.questionEl.find( answerSelector ).val();
			var answerVal2 = this.questionEl.find( answerSelector2 ).val();
			
			//jos kysymyksessä on monivalinta niin mennään tähän, silloin vähintään toisessa (checkbox tai textbox) pitää olla sisältöä että pääsee jatkamaan
			if ( this.activeQuestion.type == 5 ){
				...
				var elements = this.questionEl.find("input");
					for (var i = elements.length - 1; i >= 0; i--) {
						...
						if (elements[i].checked == true){
							answerVal = answerVal + "," + elements[i].value;
						}
						//joku muu mikä boksin sisältö, lisätään vain jos kentässä on tekstiä
						if (elements[i].name == "other" && elements[i].value != "" ){
							answerVal = answerVal + "," + elements[i].value;
						}
					};
